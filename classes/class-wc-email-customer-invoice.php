...
if ( ! class_exists( 'WC_Email_Editor_Customer_Invoice' ) ) :
 ...
 * @extends     WC_Email
 */
class WC_Email_Editor_Customer_Invoice extends WC_Email {
	...
	function get_content_html() {
		if( $this->object->has_status( 'pending' ) ){
			$email_contents = $this->get_option(
				'pending_email_contents',
				sprintf(
					__( 'An order has been created for you on {site_title}. To pay for this order please use the following link: %s {cart}', 'woocommerce' ),
					'<a href="{payment_url}">'. __( 'pay', 'woocommerce' ) .'</a>'
				)
			);
		} else {
			$email_contents = $this->get_option(
				'email_contents',
				__( 'An order has been created for you on {site_title}. {cart}', 'woocommerce' )
			);
		}
	...
	function init_form_fields() {
		$this->form_fields = array(
			...
			'pending_email_contents' => array(
				'title'			=> __( 'Pending Order Email Contents', 'woocommerce' ),
				'type'			=> 'wp_editor',
