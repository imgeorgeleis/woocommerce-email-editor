...
if ( ! class_exists( 'WC_Email_Editor_Customer_On_Hold_Order' ) ) :
 ...
 * @extends     WC_Email
 */
class WC_Email_Editor_Customer_On_Hold_Order extends WC_Email {
		...
		return wc_get_template_html( $this->template_html, array(
			...
			'email_contents'=> $this->format_string(
				apply_filters(
					...
					$this->get_option(
						'email_contents',
						__( 'Your order is on-hold until we confirm payment has been received. Your order details are shown below for your reference: {cart}', 'woocommerce' )
					)
				)
			)
		) );
